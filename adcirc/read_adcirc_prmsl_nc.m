...
% Reading adcirc prmsl from the netcdf file
% Input: path to prmsl.63.nc file
%
function [data, attrib] = read_adcirc_prmsl_nc (ncFile)

data.basedate       = datenum(ncreadatt(ncFile, 'time','base_date'),'yyyy-mm-dd HH:MM:SS');
data.time           = ncread(ncFile, 'time');
data.x              = ncread(ncFile, 'x');
data.y              = ncread(ncFile, 'y');
data.depth          = ncread(ncFile, 'depth');
try
    data.pressure       = ncread(ncFile, 'pressure');
catch
    data.pressure       = ncread(ncFile, 'pressure_min');
end
data.units          = 'm';
