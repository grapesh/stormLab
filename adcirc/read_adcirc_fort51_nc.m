...
data.const          = (ncread(ncFile, 'const'))';
data.frequency      = ncread(ncFile, 'frequency');
data.nodal_factor   = ncread(ncFile, 'nodal_factor');
data.equilibrium    = ncread(ncFile, 'equilibrium_argument');
data.amp            = ncread(ncFile, 'amp');
data.phs            = ncread(ncFile, 'phs');
...
data.dates = datenum(data.base_date) + data.time*attrib.fort15.dt/86400;
