...
% Reading adcirc time series from the netcdf file
% Input: path to maxele.61.nc file
%
function [data, attrib] = read_adcirc_fort61_nc (ncFile)
...
data.name           = (ncread(ncFile, 'station_name'))';
data.zeta           = ncread(ncFile, 'zeta');
data.base_date      = ncreadatt(ncFile,'time','base_date');
...
%data.dates = datenum(data.base_date(1:16)) + data.time*attrib.fort15.dt/86400;
data.dates = datenum(data.base_date(1:16)) + data.time/86400;
